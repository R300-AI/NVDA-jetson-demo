...
def main():
    ...
    if not args.use_dynamic:
        ...
        traced = torch.jit.trace(model_eager.to(device).eval(), example)
        torch.jit.save(traced, args.trace_out)
        print(f"[trace] 已輸出：{args.trace_out}")
        # 載入回來測速
        traced_loaded = torch.jit.load(args.trace_out).to(device)
    ...
    scripted = torch.jit.script(model_eager.to(device).eval())
    torch.jit.save(scripted, args.script_out)
    print(f"[script] 已輸出：{args.script_out}")
    scripted_loaded = torch.jit.load(args.script_out).to(device)
