...
# -----------------------------------------------------
def print_mem(tag):
    alloc = torch.cuda._____( ) / 1024**2          
    reserved = torch.cuda._____( ) / 1024**2       
...
# -----------------------------------------------------
def main():
    if not torch.cuda._____( ):                    
        print("CUDA 不可用，請確認 JetPack CUDA 安裝是否正常。")
        return

    device = torch.device("cuda")
    model = SmallCNN().to(_____)                   

    x = torch.randn(64, 3, 32, 32).to(_____)       
    y = torch.randint(0, 10, (64,)).to(_____)      
