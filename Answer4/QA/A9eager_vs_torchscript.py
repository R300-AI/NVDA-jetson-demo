...
def synthetic_data(batch=64):
    # === TODO：產生隨機輸入 ===
    return torch._____(batch, 3, 32, 32)      # randn
...
def main():
    ...
    # === TODO：trace 模型 ===
    traced = torch.jit._____( model.to(device), example )            # trace
    # === TODO：儲存 traced 模型 ===
    torch.jit._____( traced, "smallcnn_traced.ts" )                  # save
    # === TODO：載入 traced 模型 ===
    traced_loaded = torch.jit._____( "smallcnn_traced.ts" ).to(device)  # load
    ...
    # ===== JIT: script =====
    # === TODO：script 模型 ===
    scripted = torch.jit._____( model )                               # script
    # === TODO：儲存 scripted 模型 ===
