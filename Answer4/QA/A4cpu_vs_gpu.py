...
def synthetic_data(batch=64):
    x = torch._____(batch, 3, 32, 32)        # TODO：產生隨機影像 → randn
...
def train_one_epoch(model, device, use_amp=False):
    ...
    criterion = nn._____( )                 # TODO：填入 CrossEntropyLoss

    scaler = torch.cuda.amp.GradScaler(enabled=use_amp)

    x, y = synthetic_data()
    x, y = x.to(_____), y.to(_____)         # TODO：移到 CPU 或 CUDA → device
    ...
    for _ in range(100):
        ...
        scaler.scale(loss)._____( )          # TODO：backward
...
def main():
    ...
    # GPU 測試（Jetson Orin）
    if torch.cuda._____( ):                  # TODO：is_available
        gpu_time = train_one_epoch(
            SmallCNN().to("cuda"),
            torch.device("cuda"),
            use_amp=_____                    # TODO：True（啟用 AMP）
        )
