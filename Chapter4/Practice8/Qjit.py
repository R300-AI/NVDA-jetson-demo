...
def main():
    ...
    # === TODO: trace 編譯 ===
    traced = torch.jit._____( model.to(device).eval(), example )       
    # === TODO: 儲存模型 ===
    torch.jit._____( traced, "model_traced.ts" )                      
    # === TODO: 載入模型 ===
    traced_loaded = torch.jit._____( "model_traced.ts" ).to(device)   
    ...
    # ---------------- script JIT ----------------
    # === TODO: script 編譯 ===
    scripted = torch.jit._____( model.to(device).eval() )              
    # === TODO: 儲存模型 ===
    torch.jit._____( scripted, "model_scripted.ts" )                 
    # === TODO: 載入模型 ===
    scripted_loaded = torch.jit._____( "model_scripted.ts" ).to(device)   
