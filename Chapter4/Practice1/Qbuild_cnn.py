...
class SmallCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = _____( self.conv1(x) )       
        x = self.pool( x )                
        x = _____( self.conv2(x) )        
...
def count_params(m: nn.Module):
    ...
    total = sum(p._____( ) for p in m.parameters())                         
    trainable = sum(p.numel() for p in m.parameters() if p._____)           
...
def make_synthetic(batch_size: int = 64, n_classes: int = 10):
    ...
    """
    # === TODO: 產生合成影像與標籤 ===
    x = torch._____(batch_size, 3, 32, 32)         
    ...
    tfm = transforms.Compose([ transforms._____( ) ])   
...
def main():
    ...
    # === TODO: 選擇運算裝置 ===
    device = torch.device("cuda" if torch._____( ) else "cpu")  
