...
def main():
    ...
    if not args.use_dynamic:
        ...
        traced = torch.jit._____( model_eager.to(device).eval(), example )   
        torch.jit._____( traced, args.trace_out )                            
        print(f"[trace] 已輸出：{args.trace_out}")

        # === TODO：載入回測 ===
        traced_loaded = torch.jit._____( args.trace_out ).to(device)         
    ...
    scripted = torch.jit._____( model_eager.to(device).eval() )              
    torch.jit._____( scripted, args.script_out )                             
    print(f"[script] 已輸出：{args.script_out}")
    scripted_loaded = torch.jit._____( args.script_out ).to(device)          
