...
def main():
    ...
    traced = torch.jit._____(eager, example)            
    torch.jit._____(traced, "smallcnn_traced.ts")       
    traced_loaded = torch.jit._____("smallcnn_traced.ts").to(device).eval()   
    ...
    scripted = torch.jit._____(eager)                   
    torch.jit._____(scripted, "smallcnn_scripted.ts")   
    scripted_loaded = torch.jit._____("smallcnn_scripted.ts").to(device).eval()   
