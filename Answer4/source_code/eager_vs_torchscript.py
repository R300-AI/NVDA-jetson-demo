...
def main():
    ...
    torch.jit.save(traced, "smallcnn_traced.ts")
    traced_loaded = torch.jit.load("smallcnn_traced.ts").to(device)
    traced_fps = bench(traced_loaded, device)
    print(f"[Traced  ] {traced_fps:10.2f} samples/sec")

    # ===== JIT: script =====
    scripted = torch.jit.script(model)
    torch.jit.save(scripted, "smallcnn_scripted.ts")
    scripted_loaded = torch.jit.load("smallcnn_scripted.ts").to(device)
